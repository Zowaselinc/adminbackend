...
class UserbasicController {
  ...
  The Zowasel Team
    ...
    <div style="Font-family: 'Radio Canada', sans-serif; display: none; white-space: nowrap; font-size: 15px; line-height: 0; text-align: center;">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; </div>
            ...
            this phone number: ${element.phone} and Password. Temporary password: ${element.password}.Change your password for security reasons. Need help? Call +234 01 342 4403, WhatsApp +234 70 250 656 64, Email marketplace@zowasel.com. For partnerships, email partners@zowasel.com. 
            Thank you for trusting Zowasel. Best regards, Zowasel Team.`;
            ...
            if (element.email != "null" && element.phone != "null"){
              ...
              sendSmsSINGLE(element.phone, phoneMessageContent);
                ...
                await UserTypeModel.create({ ...change, ...{ user_id: user.id } }).catch(
                  ...
                  if (element.has_company) {
                    var response = await UserbasicController.saveBasicBatchCompany(user, element);
                    if (response.error) {
                        return res.status(400).json({
                            error: true,
                            message: response.message,
            ...
            var logError = await ErrorLog.create({
                error_name: "Error on creating users in batch",
                error_description: error.toString(),
        ...
        try {
            ...
            if(element.bvn != ""){
             ...
             var kycVerification = await KYC.create({
                ...
                files: JSON.stringify({front: element.id_front, back: element.id_back}),
                bvn:  userbvn,
                verified: 1
          ...
          console.log(error);
            var logError = await ErrorLog.create({
                ...
                error_description: error.toString(),
                route: "/admin/user/account/addbatchbasicuser",
                error_code: "500",
            });
            // if (logError) {
            //     return res.status(500).json({
        ...
        try {
            ...
            let checkeId = crypto.randomUUID();
             var  userkyb = await KYB.create({
                  ...
                  cac: element.cac,
                  financial_statement: element.financial_statement,
                  mou: element.mou,
                  check_id: checkeId,
                  status: "pending"
              });
            ...
            var logError = await ErrorLog.create({
                error_name: "Error on creating batch basic user account",
                error_description: error.toString(),
                route: "/api/admin/user/account/addbatchuser",
                error_code: "500",
            });
            // if (logError) {
            //     return res.status(500).json({
...
// ************************************************************************************************************************
    ...
    static async deleteBsicuser(req, res){
        ...
        var delkyc= await KYC.destroy({where : {user_id : req.params.id}});

        var delkyb= await KYB.destroy({where : {user_id : req.params.id}});
